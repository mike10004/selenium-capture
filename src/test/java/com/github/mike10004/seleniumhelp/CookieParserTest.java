...
public class CookieParserTest {
    ...
    public void parse() throws Exception {
        ...
        CookieParser parser = new CookieParser(FlexibleCookieSpec.getDefaultAttributeHandlers());
        CookieOrigin origin = CookieUtility.getInstance().buildCookieOrigin(new java.net.URL("https://www.google.com/")).getLeft();
        List<DeserializableCookie> cookies = parser.parse(new BasicHeader(HttpHeaders.SET_COOKIE, googleCookie), origin);
        assertEquals("size", 1, cookies.size());
        DeserializableCookie c = cookies.get(0);
